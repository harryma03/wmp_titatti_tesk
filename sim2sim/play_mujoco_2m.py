...
class SimulationRunner:
    ...
    def load_model(self):
        ...
        self.initial_qpos[7:7 + self.num_actions] = self.default_angles
    ...
    def build_observation(self):
        ...
        # Calculate base_lin_vel using quat_rotate_inverse equivalent
        ...
        # policy history step = 54 = ang_vel(3)+gravity(3)+dof_pos(16)+dof_vel(16)+actions(16)
        ...
        # world-model prop = 57 = ang_vel(3)+gravity(3)+cmd(3)+dof_pos(16)+dof_vel(16)+actions(16)
    ...
    def policy_inference(self):
        ...
        cmd_tensor = torch.from_numpy(self.cmd[:3]).to(self.device, dtype=torch.float32).unsqueeze(0)
        ...
        with torch.inference_mode():
            ...
            with time_block("policy推理"):
                action = self.policy(cmd_tensor, history_flat, self.wm_feature)
        ...
        with mujoco.viewer.launch_passive(self.m, self.d) as viewer:
            ...
            while viewer.is_running() and time.time() - start < self.simulation_duration:
                ...
                if self.counter % self.control_decimation == 0: # 20ms
                    ...
                    action_tensor = torch.from_numpy(self.action).to(self.device, dtype=torch.float32).view(1, 1, -1)
